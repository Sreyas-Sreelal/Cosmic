...
use log::{info,error,Metadata,Level,Record};
...
fn main() -> Result<(),log::SetLoggerError> {
    ...
    log::set_logger(&COMSIC_LOGGER)
        .map(|()| log::set_max_level(log::LevelFilter::Info))?;

    let token = std::env::var("COSMIC_TOKEN").expect("Error cannot fetch token.Make sure environment variable COSMIC_TOKEN is set");
